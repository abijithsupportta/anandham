...
export default function DharmasPage() {
  ...
  async function handleToggleStatus(dharma: Dharma) {
    ...
    const { error } = await supabase.from("dharmas").update(updates).eq("id", dharma.id);
    if (!error) {
      setDharmas((prev) =>
        prev.map((d) => (d.id === dharma.id ? { ...d, ...updates } as Dharma : d))
      );
    }
  }
  ...
  async function handleDelete(id: string) {
    const { error } = await supabase
      ...
      .eq("id", id);
    if (!error) setDharmas((prev) => prev.filter((d) => d.id !== id));
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage dharma teachings</p>
        </div>
        <Link
          href="/dharmas/new"
                ...
                <tr key={dharma.id} className="transition hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link
                        href={`/dharmas/${dharma.id}`}
                        className="rounded-md p-1.5 text-gray-500 hover:bg-gray-100"
                      >
                        <Pencil className="h-4 w-4" />
                      </Link>
                      <button
                        onClick={() => handleToggleStatus(dharma)}
                        className="rounded-md p-1.5 text-gray-500 hover:bg-gray-100"
                        title={dharma.status === "draft" ? "Publish" : "Unpublish"}
                      >
